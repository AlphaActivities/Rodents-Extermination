...
import { trackCall, trackFaqOpen, trackSectionView } from '../analytics';

const faqs = [
  {
    question: 'How do I know if my attic insulation needs replacement?',
    slug: 'insulation_replacement_signs',
...
function FAQItem({ question, slug, answer }: { question: string; slug: string; answer: string }) {
...
export default function FAQ() {
  const { ref: headerRef, visible: headerVisible } = useReveal(() => trackSectionView('faq'));
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {faqs.map((faq, i) => (
            ...
            >
              <FAQItem question={faq.question} slug={faq.slug} answer={faq.answer} />
            </div>
          ))}
        </div>
        ...
        >
          ...
          <a
            href="tel:9728046456"
            onClick={() => trackCall('faq_cta', 'faq')}
            className="btn-primary btn-luxury-glow"
          >
